...
(
    ...
    sudo nvidia-smi --power-limit=650 -i "$CUDA_VISIBLE_DEVICES"    # lock to 330 W
    sudo nvidia-smi -lgc 1980 -i "$CUDA_VISIBLE_DEVICES"            # lock to 1410 MHz.  The max on A100 is 1410 MHz
) >/dev/null
...
(
    ...
    sudo nvidia-smi --power-limit=500 -i "$CUDA_VISIBLE_DEVICES"    # lock to 330 W
) >/dev/null
